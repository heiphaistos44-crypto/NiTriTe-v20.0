...
class AdvancedInstaller:
        ...
        try:
            ...
            if winget_id and self.winget_available:
                ...
                if success:
                    ...
                    if on_log:
                        on_log("⚠️ WinGet échoué, essai Chocolatey...", "warning")
            ...
            if choco_id and self.chocolatey_available:
                ...
                if success:
                    ...
                    if on_log:
                        on_log("⚠️ Chocolatey échoué, essai téléchargement direct...", "warning")

            # Méthode 3: Téléchargement direct
            if download_url:
                if on_log:
                    on_log(f"⬇️ Téléchargement direct: {download_url}", "info")
    ...
    def _install_download(
        ...
        try:
            ...
            # Si portable, copier sur bureau et c'est fini
            if portable:
                if on_progress:
                    on_progress(0.9, "Copie sur bureau...")

                # Copier exe sur bureau
                desktop_file = self.desktop / filename
                shutil.copy2(dest_file, desktop_file)

                if on_log:
                    on_log(f"✓ Copié sur bureau: {desktop_file.name}", "success")

                if on_progress:
                    on_progress(1.0, "Terminé")

                msg = f"✓ {app_name} téléchargé (portable) → Bureau"
                logger.success("Installation", msg, path=str(desktop_file))
                return True, msg
