...
class OrdiPlusManagerDialog(ctk.CTkToplevel):
    ...
    def _create_ui(self):
        ...
        title = ctk.CTkLabel(
            main,
            text="🏢 Gérer la catégorie OrdiPlus",
    ...
    def _create_custom_pack_card(self, parent, pack_name: str, pack_data: dict):
        ...
        if apps_list:
            ...
            empty_label = ctk.CTkLabel(
                apps_frame,
                text="⚠️ Aucune application dans ce pack",
                font=(DesignTokens.FONT_FAMILY, DesignTokens.FONT_SIZE_SM),
        ...
        try:
            ...
            for tool_name in self.cleaning_tools:
                ...
                if "RustDesk" in tool_name:
                    progress_dialog.add_log(f"⬇️ Téléchargement de {tool_name}...", "info")
                ...
                if tool_name in app_paths:
                    ...
                    if exe_path:
                        ...
                        progress_dialog.add_log(f"⚠️ {tool_name} non trouvé (installez-le d'abord)", "warning")

            progress_dialog.add_log(f"✅ Dossier 'Outils de Nettoyage' créé sur le bureau", "success")

        except Exception as e:
            progress_dialog.add_log(f"⚠️ Erreur création dossier outils: {str(e)}", "warning")
