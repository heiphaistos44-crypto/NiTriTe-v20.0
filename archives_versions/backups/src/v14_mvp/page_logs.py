...
try:
    ...
    print("⚠️ win32evtlog non disponible - installation: pip install pywin32")
...
class LogsPage(ctk.CTkFrame):
    ...
    def _create_app_logs_view(self, parent):
        ...
        # Avertissements
        self._create_stat_item(stats_frame, "⚠️ Avertissements", error_counts.get("WARNING", 0), DesignTokens.INFO)
        ...
        logs_title.pack(padx=20, pady=(15, 10), anchor="w")
        ...
        # Scrollbar
        scrollbar_app = ctk.CTkScrollbar(text_frame)
        scrollbar_app.pack(side=tk.RIGHT, fill=tk.Y)

        self.app_logs_text = tk.Text(
            text_frame,
            wrap=tk.WORD,
            bg="#1e1e1e",
            fg="#00ff00",  # Vert terminal
            font=("Consolas", 10),
            relief=tk.FLAT,
            padx=15,
            pady=15,
            insertbackground="#00ff00",
            yscrollcommand=scrollbar_app.set,
            height=30
        )
        self.app_logs_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar_app.configure(command=self.app_logs_text.yview)
    ...
    def _create_quick_actions(self, parent):
        ...
        ModernButton(
            row1,
            text="🖥️ Ouvrir Observateur d'événements",
            variant="outlined",
            size="md",
    ...
    def _create_logs_display(self, parent):
        ...
        self.logs_text = tk.Text(
            ...
            wrap=tk.WORD,
            bg="#1e1e1e",
            fg="#00ff00",  # Vert terminal pour meilleure visibilité
            font=("Consolas", 10),
            relief=tk.FLAT,
            padx=15,
            pady=15,
